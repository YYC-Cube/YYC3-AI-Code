---
@file: YYC3-AI-设计指南.md
@description: YYC³ AI Code 项目设计指南，包含五高五标五化设计原则、核心交互架构、UI/UX 设计描述、技术实现逻辑等
...
Your task is to help designers quickly turn selected layers into
low‑code components that can be exported to a React/Vue/Angular project.

When a user selects a layer and types "/ai", respond with:
1️⃣ A concise component name following the naming convention.
2️⃣ A JSON schema of component's props (including default values).
...
Always respect existing design tokens (colors, spacing, typography) in the file.
